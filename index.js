...
const bannerList1 = [
    {
        ...
        ideaName: "banner",
        ideaStatus: "valid",
        ideaUrl: { originalLink: "http://www.baidu.com", viewUrl: "http://www.baidu.com", linkType: "H5_LINK" },
        ideaWords: [],
        isNotice: "0",
        nth: 1,
...
function starFireSwiper(params) {
    /**
     * css class 命名列表
    ...
    /** 滑动结束函数 */
    const slideEnd = params.slideCallback || function (index) { console.log("当前索引 >>", index); };
    /**
     * 组件节点
     * @type {HTMLElement}
     */
    ...
    function main(width, height) {
        ...
        function slideMove(slideDistance) {
            ...
            if (pagination) {
                ...
                const result = nodePagination.querySelectorAll(classNames[3])
                if (result.length) {
                    result.forEach(e => {
                        e.style.backgroundColor = 'rgba(0, 0, 0, 0.5)'
                    })
                }
                nodePaginationItems[index].style.backgroundColor = '#fff'
    ...
    function outputPagination() {
        ...
        if (!nodePagination) {
            ...
            nodePagination.style.height = '10px'
            nodePagination.style.textAlign = 'center'
            nodePagination.style.bottom = '14px'
            nodePagination.style.left = '0'
            nodePagination.style.display = 'flex'
            nodePagination.style.justifyContent = 'center'
        }
        for (let i = 0; i < nodeItems.length; i++) {
            paginations += `<div class="${classNames[3].slice(1)}" style="margin-left: 8px; width: 8px;height: 8px;background-color: rgba(0, 0, 0, 0.5);  border-radius: 50%;"></div>`;
        }
        ...
        nodePagination.querySelector(classNames[3]).style.backgroundColor = '#fff';
    }
    ...
    function initParams() {
        ...
        autoPaly = params.bannerList.length == 1 ? false : true;
        isLoop = params.bannerList.length == 1 ? false : true;
 ...
 */
function createList(className = "", starFire) {
    console.log('=> new banner');
    console.log(starFire);
    console.log('end');
    /** 页面容器节点 */
    const page = document.querySelector(starFire.starFireDom);
    if (!page) return console.warn("没有可执行的DOM！");
    let itemList = "";
    for (let i = 0; i < starFire.starFireList.length; i++) {
        itemList += `<div class="swiper_item" style="width:${starFire.starFireWidth};height:${starFire.starFireHeight};background: url(${starFire.starFireList[i].picUrl})no-repeat center / 100% 100%;float: left;  text-align: center;"></div>`;
    }
 ...
 */
function addSwiper(starFire) {
    if (starFire?.starFireList?.length == 0) return console.warn("bannerList为空！");
    ...
    const params = {
        el: ".swiper",
        bannerList: starFire.starFireList,
        starFireWidth: starFire.starFireWidth,
        starFireHeight: starFire.starFireHeight
    }
    starFireSwiper(params);
}
const params = {
    starFireDom: '.demo',
    starFireList: bannerList1,
    starFireWidth: '390px',
    starFireHeight: '300px'
}
addSwiper(params)